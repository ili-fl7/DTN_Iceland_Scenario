...
public abstract class MovementModel {
	...
	/** node's initial wait time CSV (min, max) -setting id ({@value})*/
	public static final String INIT_WAIT_TIME = "initWaitTime";
	/** simulation scenario day time */
	public static final String DAY_TIME = "dayTime";
	
	/** default day time scenario */
	public static final int DEF_DAY_TIME = 0;
	...
	/** default setting for wait time distribution */
	public static final double[] DEF_INIT_WAIT_TIMES = {0,0};
	...
	protected double minInitWaitTime;
	protected double maxInitWaitTime;

	private int maxX;
	private int maxY;
	/** Day time of the simulation scenario */
	private int dayTime;
	...
	public MovementModel(Settings settings) {
		...
		if(settings.contains(INIT_WAIT_TIME)) {
			initTimes = settings.getCsvDoubles(INIT_WAIT_TIME, 2);
		}
		else {
			initTimes = DEF_INIT_WAIT_TIMES;
		}
		...
		minInitWaitTime = initTimes[0];
		maxInitWaitTime = initTimes[1];
		checkMinAndMaxSetting(INIT_WAIT_TIME,minInitWaitTime,maxInitWaitTime);
		
		this.dayTime = settings.getInt(DAY_TIME, DEF_DAY_TIME);
		dayT = dayTime;
		System.out.print("Scenario day time is: " + this.dayTime);
	...
	public MovementModel(MovementModel mm) {
		...
		this.maxInitWaitTime = mm.maxInitWaitTime;
		this.minInitWaitTime = mm.minInitWaitTime;
		this.maxX = mm.maxX;
		this.maxY = mm.maxY;
		this.ah = mm.ah;
		this.comBus = null;
	...
	/**
	 * Generates and returns a suitable waiting time in the beginning of the Sim.
	 * (i.e. random variable whose value is between min and max of the 
	 * {@link #INIT_WAIT_TIME} setting).
	 * @return The time as a double
	 */
	protected double generateInitWaitTime() {
		if (rng == null) {
			return 0;
		}
		return (maxInitWaitTime - minInitWaitTime) * rng.nextDouble() + 
			minInitWaitTime;
	}
	...
	/**
	 * Returns the day time ID
	 * Morning=0,Noon=1,Evening=2,Night=3
	 * @return The ID of the scenario day time
	 */
	...
	public double initPathAvailable() {
		return SimClock.getTime() + generateInitWaitTime();
	}
